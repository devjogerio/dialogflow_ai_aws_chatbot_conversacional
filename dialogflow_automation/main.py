...
# Adiciona o diretório atual ao path para permitir imports relativos se executado como script
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Automação de Setup do Dialogflow ES")
    parser.add_argument(
        "--config-dir",
        type=str,
    ...
    if not project_id:
        logger.error(
            "Project ID não fornecido via argumento ou variável de ambiente DIALOGFLOW_PROJECT_ID.")
        sys.exit(1)

    # Obtém Caminho das Credenciais (Argumento > ENV > Erro)
    credentials_path = args.credentials or os.getenv(
        "GOOGLE_APPLICATION_CREDENTIALS")
    if not credentials_path:
        logger.error(
            "Caminho das credenciais não fornecido. Defina GOOGLE_APPLICATION_CREDENTIALS ou use --credentials.")
        sys.exit(1)
    ...
    try:
        ...
        logger.info(
            f"Iniciando sincronização de {len(intents_list)} intenções...")
